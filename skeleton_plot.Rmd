...
get_shp <- st_read("bones_3.shp", stringsAsFactors = F)

get_extent <- st_read("extent12x12.shp")
...
icd10 <- 
  read.csv("icd10_codes.csv", stringsAsFactors = F, na.strings = "") %>%
...
```{r}
  get_points_2 <-
    get_shp %>%
    filter(part %in% c("humerus", "C2", "C3","C4", "C5")) %>%
    st_sample(size = area) %>%
    st_sf() %>% 
